  ...
  return (
    <main className="overflow-hidden bg-white text-[#19152A]">
      <section className="hero-bg relative px-5 py-24 sm:px-8 sm:py-32">
        <div className="relative z-10 mx-auto max-w-[1180px] text-center"><div className="eyebrow mx-auto">The NEFE Business Network</div><h1 className="mx-auto mt-7 max-w-4xl text-[46px] font-semibold leading-[1.06] tracking-[-.055em] sm:text-[68px]">The operating system for <span className="gradient-text">connected business growth.</span></h1><p className="mx-auto mt-7 max-w-2xl text-base leading-8 text-[#6F6877]">NEFE connects businesses around shared customers—turning partnerships, campaigns, referrals, experiences and rewards into measurable commercial infrastructure.</p><div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row"><Link href="/onboarding" className="button-primary">Join the business network →</Link><Link href="/commerce-graph" className="button-secondary">View Commerce Graph</Link></div></div>
      </section>
      <section className="mx-auto max-w-[1240px] px-5 py-24 sm:px-8 sm:py-32"><div className="grid gap-12 lg:grid-cols-[.75fr_1.25fr]"><div><div className="eyebrow">Platform overview</div><h2 className="mt-5 text-4xl font-semibold leading-tight tracking-[-.05em] sm:text-5xl">One platform.<br/>An entire commercial ecosystem.</h2><p className="mt-5 text-[12px] leading-6 text-[#7B7481]">Most business software optimizes activity inside one company. NEFE creates value between companies.</p></div><div className="grid gap-3 sm:grid-cols-2">{capabilities.map(([title,detail],i)=><article key={title} className="rounded-[22px] border border-[#E8E3EB] p-5 transition hover:-translate-y-1 hover:border-[#CEC1F6] hover:shadow-[0_14px_35px_rgba(53,36,96,.07)]"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#F0ECFF] text-[9px] font-bold text-[#5E3BEE]">0{i+1}</span><h3 className="mt-5 text-[15px] font-semibold">{title}</h3><p className="mt-2 text-[9px] leading-5 text-[#837C88]">{detail}</p></article>)}</div></div></section>
      ...
      <section className="px-5 py-24 text-center sm:px-8 sm:py-32"><div className="mx-auto max-w-3xl"><div className="eyebrow mx-auto">See it in action</div><h2 className="mt-6 text-4xl font-semibold tracking-[-.05em] sm:text-5xl">Explore the connected platform.</h2><p className="mx-auto mt-5 max-w-xl text-[12px] leading-6 text-[#7B7481]">Move from strategy to the software experiences businesses and customers use every day.</p><div className="mt-8 flex flex-wrap justify-center gap-3"><Link href="/business-portal" className="button-primary">View Business Portal →</Link><Link href="/consumer" className="button-secondary">View Consumer App</Link><Link href="/experience-builder" className="button-secondary">Open Experience Builder</Link></div></div></section>
    </main>
  );
}
