...
import {registerInternalAccount,RegistrationUnavailableError,type RegistrationResult} from "@/app/lib/auth/internal-registration";
  ...
  const result=await registerInternalAccount({fullName:String(formData.get("fullName")??""),email,password:String(formData.get("password")??""),confirmPassword:String(formData.get("confirmPassword")??""),invitationCode:String(formData.get("invitationCode")??""),termsAccepted:formData.get("termsAccepted")==="on"},{invitationCode:serverEnv().NEFE_INTERNAL_SIGNUP_CODE,organizationSlug:serverEnv().NEFE_INTERNAL_ORGANIZATION_SLUG},{
    ...
    async createAccount(input){
      try{await database().transaction(async tx=>{
        ...
        if(existing[0])throw new Error("duplicate");
      ...
      })}catch(error){if(error instanceof RegistrationUnavailableError)throw error;if(postgresCode(error)==="23505")throw new Error("duplicate");throw error}
    },
  });
