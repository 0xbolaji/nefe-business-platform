  ...
  return <main className="executive-insights-page min-h-screen bg-[#F6F5F8] text-[#19152A]">
    ...
    <div className="mx-auto max-w-[1500px] px-4 py-8 sm:px-7 lg:px-10 lg:py-10">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div><div className="flex w-fit items-center gap-2 rounded-full border border-[#DDD4F8] bg-white px-3 py-2 text-[9px] font-bold uppercase tracking-[.14em] text-[#5E3BEE]"><span className="h-2 w-2 rounded-full bg-[#C6983E]" /> Illustrative ecosystem data</div><h1 className="mt-4 text-3xl font-semibold tracking-[-.05em] sm:text-[40px]">Executive Insights</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#827B89]">A modeled view of ecosystem growth, partner activity, and commercial impact.</p></div>
      ...
      <section className="mt-10">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{campaigns.map((campaign) => <article key={campaign.name} className="group rounded-[20px] border border-[#E8E4EC] bg-white p-4 transition hover:-translate-y-1 hover:border-[#D5CAF8] hover:shadow-[0_14px_35px_rgba(54,37,99,.07)]"><div className="flex items-center justify-end"><span className="rounded-full bg-[#EAF9F2] px-2 py-1 text-[8px] font-bold text-[#159166]">{campaign.lift}</span></div><h3 className="mt-4 text-[12px] font-semibold">{campaign.name}</h3><p className="mt-1.5 truncate text-[8px] text-[#99939F]">{campaign.partners}</p><div className="mt-4 flex items-end justify-between border-t border-[#F0EDF3] pt-3"><div><p className="text-[7px] text-[#A29CA7]">Revenue</p><p className="mt-1 text-xs font-bold">{campaign.revenue}</p></div><div className="text-right"><p className="text-[7px] text-[#A29CA7]">Conversion</p><p className="mt-1 text-xs font-bold text-[#5E3BEE]">{campaign.conversion}</p></div></div></article>)}</div>
      </section>
