...
import {
  businessContacts,businessRelationships,businessServices,businessTags,businesses,collaborationActivityEvents,
  campaignParticipants,campaigns,favorites,industries,journeyParticipants,journeyStages,journeys,notifications,
...
const activityDetail=(eventType:string,metadata:unknown)=>{const values=metadata&&typeof metadata==="object"?metadata as Record<string,unknown>:{};const parts=[values.component,values.stage,values.status].filter(value=>typeof value==="string") as string[];return parts.length?parts.map(titleCase).join(" · "):eventType.replaceAll("."," ").replace(/(^|\s)\S/g,letter=>letter.toUpperCase())};
...
export const getWorkspaceData=cache(async():Promise<WorkspaceData>=>{
  ...
  const [
    ...
    journeyRows,journeyParticipantRows,journeyStageRows,recommendationRows,notificationRows,activityRows,pilotRows,
    ...
    db.select().from(journeyParticipants).where(eq(journeyParticipants.organizationId,organizationId)),
    ...
    db.select().from(collaborationActivityEvents).where(eq(collaborationActivityEvents.organizationId,organizationId)).orderBy(desc(collaborationActivityEvents.createdAt)).limit(50),
  ...
  const workspaceJourneys:Journey[]=journeyRows.map(row=>{const stages=journeyStageRows.filter(item=>item.journeyId===row.id).map(item=>({id:item.id,title:item.title,businessId:item.businessId??"",touchpoint:item.touchpoint,status:item.status.toLowerCase()==="covered"?"covered" as const:"gap" as const}));return {id:row.id,name:row.name,journeyType:row.journeyType,objective:row.objective,targetCustomer:row.targetCustomer,intendedOutcome:row.intendedOutcome,businessIds:journeyParticipantRows.filter(item=>item.journeyId===row.id).map(item=>item.businessId),coverage:row.coverage,completion:row.completion,touchpoints:stages.length,stages}});
