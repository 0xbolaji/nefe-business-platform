...
export interface RegistrationDependencies{hashPassword(password:string):Promise<string>;createAccount(input:AtomicRegistration):Promise<void>}
...
export async function registerInternalAccount(raw:unknown,configuration:{invitationCode?:string;organizationSlug?:string},dependencies:RegistrationDependencies):Promise<RegistrationResult>{
  ...
  if(!configuration.invitationCode||!configuration.organizationSlug)return {ok:false,error:"Registration is currently unavailable."};
  if(!validCode(parsed.data.invitationCode,configuration.invitationCode))return {ok:false,error:GENERIC_REGISTRATION_ERROR};
  try{await dependencies.createAccount({name:parsed.data.fullName,email:parsed.data.email,passwordHash:await dependencies.hashPassword(parsed.data.password),organizationSlug:configuration.organizationSlug,role:INTERNAL_REGISTRATION_ROLE,registeredAt:new Date()});return {ok:true}}catch(error){if(error instanceof RegistrationUnavailableError)return {ok:false,error:"Registration is currently unavailable."};return {ok:false,error:GENERIC_REGISTRATION_ERROR}}
}
