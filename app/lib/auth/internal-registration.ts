...
export type RegistrationAvailabilityDiagnostic={attemptId:string;reason:"registration_configuration_missing"|"organization_not_found";signupCodeConfigured:boolean;organizationSlugConfigured:boolean;organizationFound:boolean};
...
export interface RegistrationDependencies{hashPassword(password:string):Promise<string>;createAccount(input:AtomicRegistration):Promise<void>;reportFailure(diagnostic:RegistrationDiagnostic):void;reportAvailability(diagnostic:RegistrationAvailabilityDiagnostic):void}
...
export async function registerInternalAccount(raw:unknown,configuration:{invitationCode?:string;organizationSlug?:string},dependencies:RegistrationDependencies,attemptId:string):Promise<RegistrationResult>{
  ...
  if(!configuration.invitationCode||!configuration.organizationSlug){dependencies.reportFailure(diagnostic(attemptId,"unexpected","configuration_error"));dependencies.reportAvailability({attemptId,reason:"registration_configuration_missing",signupCodeConfigured:Boolean(configuration.invitationCode),organizationSlugConfigured:Boolean(configuration.organizationSlug),organizationFound:false});return {ok:false,error:"Registration is currently unavailable."}}
  ...
  try{await dependencies.createAccount({name:parsed.data.fullName,email:parsed.data.email,passwordHash,organizationSlug:configuration.organizationSlug,role:INTERNAL_REGISTRATION_ROLE,registeredAt:new Date()});return {ok:true}}catch(error){
    if(error instanceof RegistrationUnavailableError){dependencies.reportFailure(diagnostic(attemptId,"organization_lookup","configuration_error"));dependencies.reportAvailability({attemptId,reason:"organization_not_found",signupCodeConfigured:Boolean(configuration.invitationCode),organizationSlugConfigured:Boolean(configuration.organizationSlug),organizationFound:false});return {ok:false,error:"Registration is currently unavailable."}}
