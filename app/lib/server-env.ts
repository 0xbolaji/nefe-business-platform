...
const schema=z.object({DATABASE_URL:z.string().url().startsWith("postgresql://").optional(),AUTH_SECRET:z.string().min(32).optional(),AUTH_URL:z.string().url().optional(),NEFE_DEMO_AUTH_ENABLED:z.enum(["true","false"]).default("false"),NEFE_DEMO_EMAIL:z.string().email().default("owner@nefe.local"),NEFE_DEMO_PASSWORD:z.string().min(12).optional(),NEFE_INTERNAL_SIGNUP_CODE:optionalValue(z.string().min(16)),NEFE_INTERNAL_ORGANIZATION_SLUG:optionalValue(z.string().min(2).max(160)),AUTH_GOOGLE_ID:z.string().optional(),AUTH_GOOGLE_SECRET:z.string().optional()});
