import "server-only";
import {z} from "zod";
const schema=z.object({DATABASE_URL:z.string().url().startsWith("postgresql://").optional(),AUTH_SECRET:z.string().min(32).optional(),AUTH_URL:z.string().url().optional(),NEFE_DEMO_AUTH_ENABLED:z.enum(["true","false"]).default("false"),NEFE_DEMO_EMAIL:z.string().email().default("owner@nefe.local"),NEFE_DEMO_PASSWORD:z.string().min(12).optional(),AUTH_GOOGLE_ID:z.string().optional(),AUTH_GOOGLE_SECRET:z.string().optional()});
