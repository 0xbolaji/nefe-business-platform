...
export default function NewPilotWorkflow(){
 ...
 const [name,setName]=useState(initial?`${initial.title} Pilot`:"");
 ...
 const create=()=>{
  ...
  startTransition(async()=>{
   const result=await createPilotRecord({id,name,opportunityId:opportunity.id,ownerId,objective,startDate,endDate,reviewDate:endDate,participants});
 ...
 return <>
  <PageHeader eyebrow="Create pilot" title="Convert an approved opportunity into an execution plan" description="Create the pilot foundation. Add milestones, KPIs, risks and approvals from its workspace."/>
  ...
  <AppCard>
   ...
   {step===2&&<><SectionHeader title="Define pilot" description="Commercial rationale carries forward from the opportunity."/><label className="ws-field"><span>Pilot name</span><input required minLength={3} maxLength={160} value={name} onChange={event=>setName(event.target.value)}/></label><label className="ws-field"><span>Pilot objective</span><textarea required value={objective} onChange={event=>setObjective(event.target.value)}/></label></>}
