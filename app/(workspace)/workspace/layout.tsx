...
export default async function WorkspaceLayout({children}:{children:React.ReactNode}) { const context=await requireWorkspaceContext();const [data,workspaces]=await Promise.all([getWorkspaceData(),getAvailableWorkspaces()]);const permissions={manage:can(context.membership.role,"workspace.settings")||can(context.membership.role,"team.manage")},creation={business:can(context.membership.role,"business.manage"),opportunity:can(context.membership.role,"opportunity.update"),campaign:can(context.membership.role,"business.manage"),journey:can(context.membership.role,"business.manage"),pilot:can(context.membership.role,"pilot.create")},workspaceRevision=[context.organization.id,...data.businesses.map(item=>item.id),...data.opportunities.map(item=>`${item.id}:${item.stage}`),...data.campaigns.map(item=>`${item.id}:${item.status}`),...data.journeys.map(item=>`${item.id}:${item.completion}`),...data.pilots.map(item=>`${item.id}:${item.status}:${item.milestones.map(milestone=>milestone.status).join(".")}`),...Object.values(data.favorites).flat()].join("|");return <PermissionProvider value={creation}><CollaborationProvider key={workspaceRevision} initialData={data}><PilotProvider initialPilots={data.pilots}><AppShell identity={{name:context.user.name,email:context.user.email,initials:context.user.name.split(" ").map(part=>part[0]).join("").slice(0,2).toUpperCase(),role:context.membership.role}} activeWorkspaceId={context.organization.id} workspaces={workspaces.length?workspaces:[{id:context.organization.id,name:context.organization.name,workspaceName:context.organization.workspaceName}]} permissions={permissions} workspaceName={context.organization.workspaceName} unreadNotifications={data.unreadNotifications}>{children}</AppShell></PilotProvider></CollaborationProvider></PermissionProvider>; }
