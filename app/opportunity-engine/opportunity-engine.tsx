...
import { RAK_SCENARIO } from "../lib/demo-scenario";
...
const categories = ["All","Hotels","Car Rentals","Restaurants","Luxury Dining","Beach Clubs","Events","Retail","Healthcare","Real Estate","Luxury Experiences","Pilot Anchors"];
const defaultNames: string[] = [...RAK_SCENARIO.coreParticipants];
const slideCount = 7;
...
export default function OpportunityEngine() {
  ...
  const [selectionFeedback, setSelectionFeedback] = useState("The four-participant anchor ecosystem is ready for modeled analysis.");
  ...
  const filtered = useMemo(() => merchants.filter(merchant => (category === "All" || category === "Pilot Anchors" ? category !== "Pilot Anchors" || merchant.ceo : merchant.category === category) && merchant.name.toLowerCase().includes(query.toLowerCase())), [category, query]);
  ...
  const ceoPilot = RAK_SCENARIO.coreParticipants.every((name) => selectedNames.includes(name));
  const pilot90DayValue = valueTotal * 3;
  const scenarioRecommendations = useMemo(() => {
    const categories = new Set(selected.map((merchant) => merchant.category));
    const contextual = categories.has("Luxury Dining") || categories.has("Restaurants")
      ? "Wellness is the clearest next partner for repeat-visit potential."
      : "A premium restaurant would complete the arrival-to-evening journey.";
    return [
      contextual,
      categories.has("Beach Clubs") ? "The beach experience strengthens destination dwell time." : "A beach club would extend the stay beyond the resort.",
      categories.has("Retail") ? "Retail is represented in the modeled journey." : "A retail partner could add a lower-friction redemption moment.",
      selected.length < 4 ? "Add another complementary participant before pilot review." : "The current ecosystem is dense enough for a controlled pilot.",
    ];
  }, [selected]);
  ...
  const downloadExecutiveReport = () => {
    downloadHtml("nefe-executive-commercial-report.html", "Executive Commercial Report", [
      ...
      { label: "Pilot recommendation", value: ceoPilot ? `Launch the ${RAK_SCENARIO.title} as the first controlled regional pilot.` : "Begin with the highest-fit hospitality, mobility and experience partners in one city." },
    ]);
  ...
  const downloadSimulationReport = () => {
    downloadHtml("nefe-90-day-pilot-simulation.html", "90-Day Pilot Simulation", [
      { label: "Pilot cluster", value: ceoPilot ? RAK_SCENARIO.coreParticipants.join(", ") : "Premium UAE commercial cluster" },
      ...
      { label: "Expected KPIs", value: `AED ${pilot90DayValue.toLocaleString()} modeled value, ${referralTotal.toLocaleString()} monthly referrals, 27% conversion and ${retentionScore}% repeat-visit potential` },
  ...
  const toggleMerchant = (merchant: Merchant) => {
    const adding = !selectedNames.includes(merchant.name);
    setSelectedNames(current => adding ? [...current, merchant.name] : current.filter(name => name !== merchant.name));
    const impact = merchant.category === "Restaurants" || merchant.category === "Luxury Dining"
      ? "referral potential and the evening journey"
      : merchant.category === "Car Rentals"
        ? "arrival coverage and journey completeness"
        : merchant.category === "Spas"
          ? "partner diversity and repeat-visit potential"
          : merchant.category === "Beach Clubs"
            ? "destination dwell time and experience depth"
            : "network diversity and modeled commercial value";
    setSelectionFeedback(`${adding ? "Adding" : "Removing"} ${merchant.name} ${adding ? "improves" : "reduces"} ${impact}.`);
  };
      ...
      {executiveModal&&<motion.div className="fixed inset-0 z-[300] grid place-items-center overflow-y-auto bg-[#0C0712]/72 p-4 backdrop-blur-md" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onMouseDown={event=>{if(event.target===event.currentTarget)closeExecutiveModal()}}>
        <motion.div role="dialog" aria-modal="true" aria-labelledby="executive-modal-title" initial={{opacity:0,y:24,scale:.98}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:15,scale:.98}} transition={{type:"spring",stiffness:280,damping:26}} className="w-full max-w-[760px] overflow-hidden rounded-[28px] border border-white/10 bg-[#F8F6FA] shadow-[0_35px_100px_rgba(9,5,17,.45)]">
          ...
          {executiveModal==="report"?<div className="p-6 sm:p-8">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded-[17px] bg-[#F1EDFF] p-5"><p className="text-[7px] font-bold uppercase text-[#7257D2]">Recommended next step</p><p className="mt-2 text-[9px] leading-5 text-[#51456D]">Approve a 90-day commercial pilot with executive sponsorship, shared campaign ownership and weekly KPI review.</p></div><div className="rounded-[17px] bg-[#FFF7E5] p-5"><p className="text-[7px] font-bold uppercase text-[#9B7428]">Pilot recommendation</p><p className="mt-2 text-[9px] leading-5 text-[#64543A]">{ceoPilot?`Launch the ${RAK_SCENARIO.title} as the first controlled regional pilot.`:"Begin with the highest-fit hospitality, mobility and experience partners in one city."}</p></div></div>
