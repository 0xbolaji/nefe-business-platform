...
export default function AdminCommandCenter() {
  ...
  return <main className="min-h-screen bg-[#0D0B13] text-[#EEEAF5]">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[246px] flex-col border-r border-white/[.07] bg-[#100D17] px-4 py-6 lg:flex">
      <div className="px-2"><Logo/></div><div className="mt-9 px-2 text-[8px] font-bold uppercase tracking-[.18em] text-white/25">Ecosystem operations</div><nav className="mt-3 space-y-1">{nav.map((item,i)=><button key={item.label} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-[11px] font-medium transition ${i===0?"bg-[#6845E4]/20 text-[#BCA8FF]":"text-white/45 hover:bg-white/[.04] hover:text-white/75"}`}><Icon name={item.icon} className="h-[17px] w-[17px]"/>{item.label}{item.badge&&<span className="ml-auto rounded-full bg-[#6744DF] px-2 py-0.5 text-[7px] text-white">{pending.length+campaignQueue.length}</span>}</button>)}</nav>
