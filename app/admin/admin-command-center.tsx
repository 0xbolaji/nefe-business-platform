  ...
  return <main className="min-h-screen bg-[#0D0B13] text-[#EEEAF5]">
    ...
    <div className="lg:pl-[246px]">
      ...
      <div className="mx-auto max-w-[1600px] px-4 py-7 sm:px-7 lg:px-8">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><div className="flex items-center gap-2 text-[8px] font-bold uppercase tracking-[.15em] text-[#A890F4]"><span className="h-2 w-2 rounded-full bg-[#C8A44C]"/>Illustrative ecosystem</div><h1 className="mt-2 text-2xl font-semibold tracking-[-.04em] sm:text-[30px]">Admin Command Center</h1><p className="mt-1.5 text-[9px] text-white/35">Explore sample monitoring and moderation workflows for the NEFE network.</p></div><div className="flex items-center gap-2"><select value={period} onChange={e=>setPeriod(e.target.value)} className="h-10 rounded-xl border border-white/10 bg-white/[.04] px-3 text-[8px] text-white/60 outline-none"><option className="bg-[#17131F]">Last 24 hours</option><option className="bg-[#17131F]">7 days</option><option className="bg-[#17131F]">30 days</option></select><button onClick={()=>notify("Operations report exported")} className="h-10 rounded-xl bg-[#6845E4] px-4 text-[8px] font-semibold text-white shadow-[0_8px_22px_rgba(95,58,210,.25)]">Export report</button></div></div>
