...
export default function ContactPage() {
  return <MarketingPage>
    ...
    <section className="marketing-section marketing-shell"><header className="marketing-section-heading"><span>Enquiries</span><h2>Choose the conversation that fits.</h2><p>Each path leads to the most relevant preparation material before a direct engagement.</p></header><div className="contact-options">{enquiries.map(([title,description,href])=><article key={title} className="contact-option"><h3>{title}</h3><p>{description}</p><Link href={href}>Continue with {title.toLowerCase()} →</Link></article>)}</div></section>
  </MarketingPage>;
}
