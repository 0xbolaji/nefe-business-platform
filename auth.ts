...
const credentialProvider=Credentials({
  ...
  async authorize(raw){
    ...
    const [user]=await database().select({id:users.id,name:users.name,email:users.email,passwordHash:users.passwordHash,disabledAt:users.disabledAt,securityVersion:users.securityVersion}).from(users).where(sql`lower(${users.email}) = ${parsed.data.email}`).limit(1);
    if(!user?.passwordHash||user.disabledAt||!(await compare(parsed.data.password,user.passwordHash))){logServerFailure({event:"authentication.failed",category:user?.disabledAt?"disabled_account":"invalid_credentials",userId:user?.id});return null}
    return {id:user.id,email:user.email,name:user.name,securityVersion:user.securityVersion};
  },
});
